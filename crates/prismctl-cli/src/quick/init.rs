...
use prismctl_i18n::{current_locale, keys, t, tf, Locale};
use std::io;
...
fn help_quick_init() -> String {
    match current_locale() {
        Locale::ZhCN => [
            "prismctl i（quick init）",
            "",
            "用法：",
            "  prismctl i [-t|--tool <claude|codex|gemini|all>] [-p|--provider <NAME>] [-k|--api-key <KEY>] [--lang <zh-CN|en>] [--home <PATH>] [--dry-run|--apply] [-s|--silent]",
            "",
            "说明：",
