...
use prismctl_i18n::{keys, t, tf, Locale};
use std::env;
use std::io;
...
fn run() -> Result<(), CliFailure> {
    ...
    // subcommand flags like `prismctl init ... --lang en` (template language).
    let lang_override = take_leading_kv_flag(&mut args, "--lang")?;
    if env::var_os("PRISMCTL_LANG").is_none() {
    ...
    if args.is_empty() {
        return enter_interactive_or_fail("prismctl");
    }
    ...
    // `prismctl config <...>` mirrors all commands: `prismctl config init ...` ≡ `prismctl init ...`.
    if cmd == "config" {
        if rest.is_empty() {
            return enter_interactive_or_fail("prismctl config");
        }
