...
error-non-tty-interactive-title = Error: non-interactive environment detected (stdin/stdout is not TTY); cannot enter Ekko interactive wizard.
...
error-non-tty-interactive-solution-menu =   - Run `ekko` or `ekko config` in an interactive terminal to enter the wizard
error-non-tty-interactive-solution-subcmd =   - Or use subcommands (script/CI), e.g. `ekko init --tool all` / `ekko update --tool all`
error-non-tty-solution-args =   - Or pass flags explicitly (script/CI), e.g. `--tool codex` / `--name xxx` / `--yes`
error-non-tty-solution-help =   - Full help: `ekko --help`
...
error-home-not-found = Error: cannot determine HOME. Set EKKO_HOME (or system HOME/USERPROFILE)
...
codex-provider-default-set = Set Codex default model_provider to: ekko
codex-provider-preset = Using provider preset: { $provider }
codex-auth-key-hidden = Will write Codex auth.json key: EKKO_CODEX_API_KEY (value hidden)
