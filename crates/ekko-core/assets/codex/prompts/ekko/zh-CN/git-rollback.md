...
# examples:
#   - /prompts:git-rollback                # 全交互模式，dry‑run
#   - /prompts:git-rollback --branch dev   # 直接选 dev，其他交互
...
| 场景                                            | 调用示例                                                         |
| ----------------------------------------------- | ---------------------------------------------------------------- |
| 热修补丁上线后发现 bug，需要回到 Tag `v1.2.0`   | `/prompts:git-rollback --branch release/v1 --target v1.2.0 --mode reset` |
| 运维同事误推了 debug 日志提交，需要生成反向提交 | `/prompts:git-rollback --branch main --target 3f2e7c9 --mode revert`     |
| 调研历史 bug，引导新人浏览分支历史              | `/prompts:git-rollback` （全交互，dry‑run）                              |
