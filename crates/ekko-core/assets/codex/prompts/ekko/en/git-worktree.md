---
description: Manage Git worktrees in project-level ../.ekko/project-name/ directory with smart defaults, IDE integration and content migration
...
Manage Git worktrees with smart defaults, IDE integration and content migration in structured `../.ekko/project-name/` paths.
...
| Option             | Description                                            |
| ------------------ | ------------------------------------------------------ |
| `add [<path>]`     | Add new worktree in `../.ekko/project-name/<path>`      |
...
2. **Smart Path Management**
   ...
   - Create worktrees in structured `../.ekko/project-name/<path>` directory
...
WORKTREE_BASE="$MAIN_REPO_PATH/../.ekko/$PROJECT_NAME"

# Always use absolute path to prevent nesting issues
ABSOLUTE_WORKTREE_PATH="$WORKTREE_BASE/<path>"
```

**Critical Fix**: Always use absolute paths when creating worktrees from within existing worktrees to prevent path nesting issues like `../.ekko/project/.ekko/project/path`.
...
6. **Safety Features**
   ...
   - **Absolute path enforcement**: Prevent nested `.ekko` directories when in worktree
...
```
✅ Worktree created at ../.ekko/project-name/feature-ui
✅ Copied .env
✅ Copied .env.local
📋 Copied 2 environment file(s) from .gitignore
🖥️ Open ../.ekko/project-name/feature-ui in IDE? [y/n]: y
🚀 Opening ../.ekko/project-name/feature-ui in VS Code...
```
