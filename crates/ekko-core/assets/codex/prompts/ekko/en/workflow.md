...
- Store plan summary (with context and plan) in project root directory `.ekko/plan/current/task-name.md`
...
- Naming task plan documents during archiving (when moving from `.ekko/plan/current/` to `.ekko/plan/history/`)
...
- Store execution plan in project root directory `.ekko/plan/current/task-name.md`
...
- After task completion, move plan file from `.ekko/plan/current/` to `.ekko/plan/history/` for archiving
