...
error-non-tty-interactive-title = 错误: 检测到非交互式环境（stdin/stdout 不是 TTY），无法进入 Prismctl 交互式向导。
...
error-non-tty-interactive-solution-menu =   - 在交互式终端中运行 `prismctl` 或 `prismctl config` 进入向导
error-non-tty-interactive-solution-subcmd =   - 或改用子命令（适用于脚本/CI），例如：`prismctl init --tool all` / `prismctl update --tool all`
error-non-tty-solution-args =   - 或显式补全参数（适用于脚本/CI），例如：`--tool codex` / `--name xxx` / `--yes`
error-non-tty-solution-help =   - 查看完整帮助：`prismctl --help`
...
error-home-not-found = 错误: 无法确定 HOME。请设置 PRISMCTL_HOME（或系统 HOME/USERPROFILE）
...
# Quick init/update（别名：`prismctl i`、`prismctl u`）
...
codex-provider-default-set = 将 Codex 的默认 model_provider 设置为: prismctl
