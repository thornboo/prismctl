...
```bash
/prompts:workflow <TASK_DESCRIPTION>
```
...
- Store plan summary (with context and plan) in project root directory `.prismctl/plan/current/task-name.md`
...
- Naming task plan documents during archiving (when moving from `.prismctl/plan/current/` to `.prismctl/plan/history/`)
...
- Store execution plan in project root directory `.prismctl/plan/current/task-name.md`
...
- After task completion, move plan file from `.prismctl/plan/current/` to `.prismctl/plan/history/` for archiving
