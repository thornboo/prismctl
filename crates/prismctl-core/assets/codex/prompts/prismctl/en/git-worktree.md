---
description: Manage Git worktrees in project-level ../.prismctl/project-name/ directory with smart defaults, IDE integration and content migration
...
# examples:
#   - /prompts:git-worktree add feature-ui                     # create new branch 'feature-ui' from main/master
#   - /prompts:git-worktree add feature-ui -o                  # create worktree and open directly in IDE
#   - /prompts:git-worktree add hotfix -b fix/login -o         # create new branch 'fix/login' with path 'hotfix'
#   - /prompts:git-worktree migrate feature-ui --from main     # migrate uncommitted content from main to feature-ui
#   - /prompts:git-worktree migrate feature-ui --stash         # migrate current stash to feature-ui
---

# Codex Prompt: Git Worktree

Manage Git worktrees with smart defaults, IDE integration and content migration in structured `../.prismctl/project-name/` paths.
...
```bash
# Basic operations
/prompts:git-worktree add <path>                           # create new branch named <path> from main/master
/prompts:git-worktree add <path> -b <branch>               # create new branch with specified name
/prompts:git-worktree add <path> -o                        # create and open directly in IDE
/prompts:git-worktree list                                 # show all worktree status
/prompts:git-worktree remove <path>                        # remove specified worktree
/prompts:git-worktree prune                                # clean invalid worktree references

# Content migration
/prompts:git-worktree migrate <target> --from <source>     # migrate uncommitted content
/prompts:git-worktree migrate <target> --stash             # migrate stash content
```
...
| Option             | Description                                            |
| ------------------ | ------------------------------------------------------ |
| `add [<path>]`     | Add new worktree in `../.prismctl/project-name/<path>`      |
...
2. **Smart Path Management**
   ...
   - Create worktrees in structured `../.prismctl/project-name/<path>` directory
...
WORKTREE_BASE="$MAIN_REPO_PATH/../.prismctl/$PROJECT_NAME"

# Always use absolute path to prevent nesting issues
ABSOLUTE_WORKTREE_PATH="$WORKTREE_BASE/<path>"
```

**Critical Fix**: Always use absolute paths when creating worktrees from within existing worktrees to prevent path nesting issues like `../.prismctl/project/.prismctl/project/path`.
...
6. **Safety Features**
   ...
   - **Absolute path enforcement**: Prevent nested `.prismctl` directories when in worktree
...
/prompts:git-worktree migrate feature-ui --from main
/prompts:git-worktree migrate hotfix --from ../other-worktree

# Migrate stash content
/prompts:git-worktree migrate feature-ui --stash
```
...
# Basic usage
/prompts:git-worktree add feature-ui                       # create new branch 'feature-ui' from main/master
/prompts:git-worktree add feature-ui -b my-feature         # create new branch 'my-feature' with path 'feature-ui'
/prompts:git-worktree add feature-ui -o                    # create and open in IDE directly

# Content migration scenarios
/prompts:git-worktree add feature-ui -b feature/new-ui     # create new feature worktree
/prompts:git-worktree migrate feature-ui --from main       # migrate uncommitted changes
/prompts:git-worktree migrate hotfix --stash               # migrate stash content

# Management operations
/prompts:git-worktree list                                 # view all worktrees
/prompts:git-worktree remove feature-ui                    # remove unneeded worktree
/prompts:git-worktree prune                                # clean invalid references
```

**Example Output**:

```
✅ Worktree created at ../.prismctl/project-name/feature-ui
✅ Copied .env
✅ Copied .env.local
📋 Copied 2 environment file(s) from .gitignore
🖥️ Open ../.prismctl/project-name/feature-ui in IDE? [y/n]: y
🚀 Opening ../.prismctl/project-name/feature-ui in VS Code...
```
