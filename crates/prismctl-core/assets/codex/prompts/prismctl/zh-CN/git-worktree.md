---
description: 管理 Git worktree，在项目平级的 ../.prismctl/项目名/ 目录下创建，支持智能默认、IDE 集成和内容迁移
...
# examples:
#   - /prompts:git-worktree add feature-ui                     # 从 main/master 创建新分支 'feature-ui'
#   - /prompts:git-worktree add feature-ui -o                  # 创建 worktree 并直接用 IDE 打开
#   - /prompts:git-worktree add hotfix -b fix/login -o         # 创建新分支 'fix/login'，路径为 'hotfix'
#   - /prompts:git-worktree migrate feature-ui --from main     # 将主分支未提交内容迁移到 feature-ui
#   - /prompts:git-worktree migrate feature-ui --stash         # 将当前 stash 迁移到 feature-ui
---

# Codex Prompt: Git Worktree

管理 Git worktree，支持智能默认、IDE 集成和内容迁移，使用结构化的 `../.prismctl/项目名/` 路径。
...
```bash
# 基本操作
/prompts:git-worktree add <path>                           # 从 main/master 创建名为 <path> 的新分支
/prompts:git-worktree add <path> -b <branch>               # 创建指定名称的新分支
/prompts:git-worktree add <path> -o                        # 创建并直接用 IDE 打开
/prompts:git-worktree list                                 # 显示所有 worktree 状态
/prompts:git-worktree remove <path>                        # 删除指定的 worktree
/prompts:git-worktree prune                                # 清理无效 worktree 记录

# 内容迁移
/prompts:git-worktree migrate <target> --from <source>     # 迁移未提交内容
/prompts:git-worktree migrate <target> --stash             # 迁移 stash 内容
```
...
| 选项               | 说明                                         |
| ------------------ | -------------------------------------------- |
| `add [<path>]`     | 在 `../.prismctl/项目名/<path>` 添加新的 worktree |
...
2. **智能路径管理**
   ...
   - 在结构化的 `../.prismctl/项目名/<path>` 目录创建 worktree
...
**关键修复**: 在现有 worktree 内创建新 worktree 时，始终使用绝对路径以防止出现类似 `../.prismctl/project/.prismctl/project/path` 的路径嵌套问题。
...
6. **安全特性**
   - **路径冲突防护**: 创建前检查目录是否已存在
   - **分支检出验证**: 确保分支未被其他地方使用
   - **绝对路径强制**: 防止在 worktree 内创建嵌套的 `.prismctl` 目录
...
# 基本用法
/prompts:git-worktree add feature-ui                       # 从 main/master 创建新分支 'feature-ui'
/prompts:git-worktree add feature-ui -b my-feature         # 创建新分支 'my-feature'，路径为 'feature-ui'
/prompts:git-worktree add feature-ui -o                    # 创建并直接用 IDE 打开

# 内容迁移场景
/prompts:git-worktree add feature-ui -b feature/new-ui     # 创建新功能 worktree
/prompts:git-worktree migrate feature-ui --from main       # 迁移未提交改动
/prompts:git-worktree migrate hotfix --stash               # 迁移 stash 内容

# 管理操作
/prompts:git-worktree list                                 # 查看所有 worktree
/prompts:git-worktree remove feature-ui                    # 删除不需要的 worktree
/prompts:git-worktree prune                                # 清理无效引用
```

**示例输出**:

```
✅ Worktree created at ../.prismctl/项目名/feature-ui
✅ 已复制 .env
✅ 已复制 .env.local
📋 已从 .gitignore 复制 2 个环境文件
🖥️ 是否在 IDE 中打开 ../.prismctl/项目名/feature-ui？[y/n]: y
🚀 正在用 VS Code 打开 ../.prismctl/项目名/feature-ui...
```
